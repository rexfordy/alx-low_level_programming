 ...
 *
 * Description: here, if the file is not an ELF file - exit code 98.
 */
void check_elf(unsigned char *e_ident)
{
	int index;
 ...
 * @e_ident: this is a pointer to an array containing the ELF magic numbers.
 *
 * Description: this is a Magic numbers are separated by spaces.
 */
void print_magic(unsigned char *e_ident)
{
	int index;
 ...
 *
 * Description: here, if the file cannot be closed - exit code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
 ...
 * Return: 0 on success.
 *
 * Description: Here, if the file is not an ELF File or
